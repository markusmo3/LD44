...
namespace UnityEditor
{
    ...
    internal class RuleTileEditor : Editor
    {
        ...
        public void OnEnable()
        {
            ...
            m_ReorderableList.onAddDropdownCallback = OnAddDropdown;
        }

        private void OnAddDropdown(Rect buttonrect, ReorderableList list) {
            var menu = new GenericMenu();
            menu.AddItem(new GUIContent("Duplicate"), false, duplicateClickHandler);
            menu.ShowAsContext();
        }

        private void duplicateClickHandler() {
            var index = m_ReorderableList.index;
            if (index < 0 || index >= tile.m_TilingRules.Count) {
                return;
            }
            var activeRule = tile.m_TilingRules[index];
            tile.m_TilingRules.Add((RuleTile.TilingRule) activeRule.Clone());
        }
