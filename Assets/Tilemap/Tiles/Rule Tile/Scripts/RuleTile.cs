...
namespace UnityEngine
{
    ...
    public class RuleTile : TileBase
    {
        ...
        [Serializable]
        public class TilingRule : ICloneable
        {
            ...
            public TilingRule(int[] mNeighbors, Sprite[] mSprites, GameObject mGameObject, float mAnimationSpeed, float mPerlinScale, Transform mRuleTransform, OutputSprite mOutput, Tile.ColliderType mColliderType, Transform mRandomTransform) {
                m_Neighbors = mNeighbors;
                m_Sprites = mSprites;
                m_GameObject = mGameObject;
                m_AnimationSpeed = mAnimationSpeed;
                m_PerlinScale = mPerlinScale;
                m_RuleTransform = mRuleTransform;
                m_Output = mOutput;
                m_ColliderType = mColliderType;
                m_RandomTransform = mRandomTransform;
            }
            ...
            public object Clone() {
                return new TilingRule((int[]) m_Neighbors.Clone(), (Sprite[]) m_Sprites.Clone(), m_GameObject, m_AnimationSpeed,
                    m_PerlinScale, m_RuleTransform, m_Output, m_ColliderType, m_RandomTransform);
            }
        }
                ...
                if (RuleMatches(rule, ref neighboringTiles, ref transform))
                {
                    switch (rule.m_Output)
                    {
                        ...
                        case TilingRule.OutputSprite.Random:
                            int index = Mathf.Clamp(Mathf.FloorToInt(GetPerlinValue(position, rule.m_PerlinScale, 10f) * rule.m_Sprites.Length), 0, rule.m_Sprites.Length - 1);
